...
import pickle, os.path, httplib2
...
class GoogleSheets:
  ...
  def load_credentials(self):
    ...
    return service_account.Credentials.from_service_account_file(creds_abs_path, scopes=SCOPES)
  ...
  def write_event(self, iso_time, serial_number, extra_fields):
    ...
    try:
      ...
      if self.debug:
        print('GoogleSheets: {0} cells updated.'.format(
          result \
          .get('updates') \
          .get('updatedCells'))
        )
