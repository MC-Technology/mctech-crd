...
class MediaPlayer:
  ...
  def play_media(self):
    ...
    if self.player and self.player.get_source() == VIDEO_MAIN:
      logger.info('play_media, playing {} for 30 seconds from {} '.format(VIDEO_SEC, self.video_possec))
      ...
      def timeoutHandler():
        ...
        logger.info('play_media, playing {} again from {}'.format(VIDEO_MAIN, self.video_posmain))
        self.player.quit()
        self.player = None
